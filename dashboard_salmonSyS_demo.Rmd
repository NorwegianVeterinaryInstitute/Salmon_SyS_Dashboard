...
source(here("scripts", "data_salmonSyS_demo.r"), encoding = "UTF-8")
...
bas_mort_palette <- c(
  "Baseline mortality < 2%" = "#3182bd",
  "High mortality > 2%" = "#de2d26",
  "Innacurate counts (exclusion)" = "#636363",
  "Weight upon stocking at sea > 500g (exclusion)" = "#bdbdbd"
)
...
map =
  ...
  tm_shape(bas_mort_map) +
  tm_dots(
    size = 0.15,
    fill = "group",
    fill.scale = tm_scale(values = bas_mort_palette),
    fill_alpha = 0.9,
    popup.vars = c(
      "location",
      "group",
...
dygraph(ts_no, "Mortality Norway") %>%
  ...
  dySeries("p_hw.upr", label = "Prediction interval", color = "#FFDB6D", fillGraph = TRUE) %>%
  dyOptions(fillAlpha = 0.2) %>%
  dySeries("p_hw.fit", label = "Expected", color = "#C4961A") %>%
  dySeries("p_hw.lwr", label = "Lower", color = "#FFDB6D") %>%
  dyAxis("y", label = "Monthly deaths per 1,000 fish") %>%
